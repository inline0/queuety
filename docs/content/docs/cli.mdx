...
Export a workflow's full execution history to JSON. Includes the workflow row, all jobs, events, and log entries.
...
Replay an exported workflow from a JSON file. Creates a new workflow with the exported state and step definitions.
