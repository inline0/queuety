...
`status` prints the current step, optional step name, version/hash metadata, wait mode, and a `WaitInfo` JSON blob when the workflow is currently blocked on signals or dependencies.
