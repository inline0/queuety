...
| `repeat_while( string $target_step, string $state_key, mixed $expected = true, ?string $name = null )` | Jump back to an earlier named step while a public state key matches the expected value. |
| `repeat_until( string $target_step, string $state_key, mixed $expected = true, ?string $name = null )` | Jump back to an earlier named step until a public state key matches the expected value. |
...
### `Queuety::workflow_timeline( $workflow_id )`
...
| `config()` | Optional configuration. Supports `max_attempts` and `backoff`. |
