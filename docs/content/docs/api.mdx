...
| `wait_for_signal( string $name, ?string $result_key = null, ?string $step_name = null )` | Add a signal wait step. If `result_key` is set, the payload is stored under that key instead of merging into top-level state. |
| `wait_for_signals( array $signal_names, WaitMode $mode = WaitMode::All, ?string $result_key = null, ?string $name = null )` | Wait for all or any of several signals. |
| `await_approval( string $signal_name = 'approval', ?string $result_key = 'approval', ?string $name = null )` | Wait for a human approval signal. |
| `await_input( string $signal_name = 'input', ?string $result_key = 'input', ?string $name = null )` | Wait for a human input signal. |
