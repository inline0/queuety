...
If you want the DB-backed runtime without the WordPress plugin layer, see [Standalone Use](/docs/features/standalone-use). That path uses the same PDO connection and runtime tables, but it does not include the WordPress hook bridges or WP-CLI registration.
