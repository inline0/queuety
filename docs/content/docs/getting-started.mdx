...
On activation, Queuety creates its database tables (`queuety_jobs`, `queuety_workflows`, `queuety_logs`, `queuety_schedules`) and installs a bootstrap file that allows workers to run without booting WordPress.
