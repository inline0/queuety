...
On activation, Queuety creates its database tables (`queuety_jobs`, `queuety_workflows`, `queuety_logs`, `queuety_schedules`, `queuety_signals`, `queuety_chunks`, `queuety_queue_states`, `queuety_webhooks`) so workers can claim jobs directly from MySQL.
