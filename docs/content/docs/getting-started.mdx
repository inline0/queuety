...
On activation, Queuety creates or upgrades its database tables (`queuety_jobs`, `queuety_workflows`, `queuety_logs`, `queuety_schedules`, `queuety_queue_states`, `queuety_webhooks`, `queuety_locks`, `queuety_signals`, `queuety_batches`, `queuety_chunks`, `queuety_workflow_events`) so workers can claim jobs directly from MySQL.

By default, the plugin also schedules a one-shot worker through WordPress cron every minute. That gives you a no-shell baseline that can still process jobs and workflows. For heavier workloads, move to dedicated `wp queuety work` processes.
