...
- Queuety state machines own the session lifecycle
- Queuety workflows do background work that benefits from retries, fan-out, and joins
- Neuron handles the actual agent calls and conversation memory

This is a good fit for:

- support copilots
- internal research assistants
- editorial agents that may pause for human review
- chat threads that move between planning, waiting, and response phases

## Lifecycle shape

In this example, one support session moves through these states:
...
The machine keeps the lifecycle explicit, while a background workflow gathers research and signals the session when it is done.
...
The heavy lifting is still workflow-shaped:

- planning tasks
- spawning specialist agent runs
- waiting for them
- synthesizing results

The long-lived support conversation is state-machine-shaped:

- waiting for the next user turn
- tracking whether research is running
- deciding what events are valid in each lifecycle phase
