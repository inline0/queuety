...
## Why this is cleaner than a workflow loop

The earlier workflow-loop shape worked, but the actual problem here is lifecycle-oriented:

- a draft is waiting for human review
- a reviewer can approve or reject it
- rejection returns the session to drafting
- approval finishes the session

That maps more directly to a state machine than to a linear workflow with a back-edge.

Workflow state is still excellent for orchestration facts:
