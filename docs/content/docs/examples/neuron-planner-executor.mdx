...
The pattern is straightforward. A Queuety planner step decides what work exists, `spawn_agents()` turns those tasks into independent top-level workflows, each child workflow runs a Neuron agent, and `await_agents()` joins the results back into the parent workflow.

## Why this split works well

Neuron is good at switching providers with minimal code changes and encapsulating the prompt, tools, and memory for a specialist agent. Queuety is good at durable fan-out and joining, retries, resumability, and the inspection or human-review surfaces around the run.
...
Each agent run would stop being individually inspectable, a partial crash would likely restart the whole batch, and there would be no clean join point for later stages.
