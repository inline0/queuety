...
These waits pair especially well with [Async Handoffs](/docs/workflows/async-handoffs), where one workflow first spawns independent child workflows and then joins them later.
...
- `await_workflows()` for several workflows with `all`, `any`, or `quorum` semantics
- `await_workflow_group()` when the workflow IDs come from a previously spawned group
...
## Wait on a spawned workflow group

If a previous `spawn_workflows()` or `spawn_agents()` step stored a durable group key, you can wait on that group directly instead of plumbing workflow IDs through public state yourself.
