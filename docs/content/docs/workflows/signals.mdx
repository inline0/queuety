...
Queuety::signal( $workflow_id, 'approval', [
    'approved'    => true,
...
The `HandleDecisionHandler` step reads `$state['approval']` and either publishes or archives the draft.
...
Each signal wait step is independent. You can mix `wait_for_signal()`, `wait_for_signals()`, `await_approval()`, and `await_input()` in the same workflow.
