...
Get the ordered list of events for a workflow:
...
Limit the output when a run has a longer history:

```bash
wp queuety workflow timeline 42 --limit=100 --offset=100
```
...
// Get the timeline, optionally in pages
$events = $log->get_timeline( $workflow_id );
$next   = $log->get_timeline( $workflow_id, 100, 100 );
