...
- **Reserved keys** that start with `_` (such as `_steps`, `_queue`, `_priority`, `_max_attempts`, `_on_cancel`, `_prune_state_after`, `_step_outputs`)
- **Initial payload keys** passed to `dispatch()` are not tracked as step outputs, so they are never pruned
- **The N most recent step outputs** as configured by the threshold
