...
/**
 * Tests for run-workflow dispatching and parent workflow coordination.
 */
class RunWorkflowTest extends IntegrationTestCase {
	...
	public function test_run_workflow_dispatched_and_parent_waits(): void {
		...
		$wf_id = Queuety::workflow( 'parent_with_sub' )
			...
			->run_workflow( 'sub_task', $sub_builder )
		...
		// Step 1 is a run_workflow. The next job should be the __queuety_run_workflow placeholder.
		$placeholder = $this->queue->claim();
		$this->assertNotNull( $placeholder );
		$this->assertSame( '__queuety_run_workflow', $placeholder->handler );
		$this->assertSame( 1, $placeholder->step_index );
		$this->assertSame( $wf_id, $placeholder->workflow_id );

		// Process the placeholder: this dispatches the run-workflow.
		$this->worker->process_job( $placeholder );

		// Parent should still be at step 1 (waiting for run-workflow).
		$status = $this->workflow_mgr->status( $wf_id );
		$this->assertSame( 1, $status->current_step );
		$this->assertSame( WorkflowStatus::Running, $status->status );

		// There should now be a run-workflow in the database.
		...
		$this->assertNotNull( $sub_wf, 'Run-workflow should exist.' );
	...
	public function test_parent_advances_when_sub_completes(): void {
		...
		// Step 1: placeholder triggers run-workflow.
		$this->process_one();

		// Process run-workflow step 0 (AccumulatingStep in the sub).
		$this->process_one();

		// Run-workflow should be completed now.
		...
		$this->assertTrue( $sub_completed, 'Run-workflow should be completed.' );
	...
	public function test_run_workflow_state_merges_into_parent(): void {
		...
		// Step 0.
		$this->process_one();
		// Step 1: run-workflow placeholder.
		$this->process_one();
		// Run-workflow step 0: AccumulatingStep sets counter=1.
		$this->process_one();

		// Parent should now have the run-workflow's state merged in.
		$parent_status = $this->workflow_mgr->status( $wf_id );
		$this->assertSame( 1, $parent_status->state['counter'] );
		$this->assertSame( 'User #30', $parent_status->state['user_name'] );

		// Process step 2: AccumulatingStep in the parent.
		// It should see counter=1 from run-workflow and increment to 2.
		$this->process_one();
	...
	public function test_run_workflow_as_last_step_completes_parent(): void {
		...
		// Parent workflow should be completed since run_workflow is the last step.
		$parent_status = $this->workflow_mgr->status( $wf_id );
		$this->assertSame( WorkflowStatus::Completed, $parent_status->status );
		$this->assertSame( 1, $parent_status->state['counter'] );
	}
